...
app.get('/posts/:index', function(req, res) {
    res.render('post', { post: posts[req.params.index] });
});

app.get('/post/:postTitle', function(req, res) {
    // now we go through all the posts stored in the posts list
    let reqPostTitle = lodash.lowerCase(req.params.postTitle);
    for (let i = 0; i < posts.length; i++) {
        if (reqPostTitle === lodash.lowerCase(posts[i].title)) {
